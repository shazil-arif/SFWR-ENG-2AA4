...
$r$: \wss{What is the type of the state variables?} : $Z$\\
$c$: \wss{What is the type of the state variables?} : $Z$\\
...
\begin{itemize}
\item \wss{What should go here?} Transition : \begin{enumerate}
\item $r := r + \Delta r$
\item $c := c + \Delta c$
\end{enumerate}
...
Seq2D($S$, scl):
\begin{itemize}
\item transition: \wss{Fill in the transition.} \begin{enumerate}
\item $s := S$
\item $scale := scl$
\end{enumerate}
...
\item exception: \wss{Fill in the exception.  One should be generated if the
    ...
    exc $:=$ (scl $<$ 0) $\lor$ (S = $<>$) $\lor$ ($| S[0] | = 0$) $\lor$ ($\exists (x \in S[ 1..|S|-1 ].  |x| \neq | S[0] |$))
\end{itemize}
...
\item exception: \wss{Generate an exception if the point lies outside of the
    map.}\\
    exc $:=$ (p.row() $ >= $ nRow) $\lor$ (p.col() $ >= $ nCol) $\lor$ (p.row() $ < $ 0) $\lor$ (p.col() $ < $ 0)
\end{itemize}
