...
\noindent checkWin()
\begin{itemize}
\item transition: if $|moves| >= TARGET$ then c.exit() else $moves := moves - 1$
\end{itemize}
...
\noindent updateData()
\begin{itemize}
\item transition: if $(moveCount) = 0$ then c.exit() else $c := $ modify the screen to update the user of how many moves are left using c.printMsg()
\end{itemize}


\noindent introMsg(b)
\begin{itemize}
\item transition: $c :=$ use the printMsg() method defined for state variable c to modify the screen to print an interact intro message to the game and state the rules(including instruction for how to enter input and how to win)
\end{itemize}
